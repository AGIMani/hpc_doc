...
!!! question "一定要有公网 IP 吗"
    ...
    如果后续接入内网 / VPN（例如 [VPN 方案](#后续可选接入-vpn)），可以进一步限制 SSH 的来源 IP。
...
如果集群后续部署 VPN，可以做到「不暴露公网 SSH」：

1. 安全组只放行 VPN 端口（如 UDP `51820`），关闭公网 `22`；
2. 用户先连 VPN，再通过内网地址 SSH 到服务器；
3. 此时监控面板也可以直接用内网地址访问，不再需要隧道。

在此之前，**SSH 隧道是当前推荐的访问方式**。
